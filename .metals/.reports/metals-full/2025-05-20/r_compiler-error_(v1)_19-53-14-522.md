  ...
  last tree to typer: Ident(_CURSOR_parse)
   ...
   symbol definition: val <error: <none>>: <error> (a TermSymbol)
      symbol package: <empty>
       symbol owners: value <error: <none>> -> value confRepo -> method run -> object MainApp
...
== Source file context for tree position ==
   ...
   121     )
   122 
   123     val confRepo: Repository[Conference] = new CsvRepository[IO, Conference](
   124       _CURSOR_parse = {
   125         case List(n, r, tops) => r.toDoubleOption.map(rd => Conference(n, rd, tops.split(";").toList))
   126         case _ => None
   127       },
