...
### scala.reflect.internal.FatalError: 
  ...
  last tree to typer: Function(value $anonfun)
       tree position: line 123 of file:///C:/Users/toorr/v1/src/main/scala/MainApp.scala
            tree tpe: Boolean => cats.effect.IO[Unit]
              symbol: value $anonfun
   symbol definition: val $anonfun: <notype> (a TermSymbol)
      symbol package: <empty>
       symbol owners: value $anonfun -> value $anonfun -> value $anonfun -> value $anonfun -> method appLoop -> method run -> object MainApp
           call site: <none> in <none>

== Source file context for tree position ==

   120                 } yield ()
   121                 case Role.Admin   => AdminMenu.run(profRef, profRepo)
   122               }
   123       cont <- readContinue
   124       _    <- if(cont) appLoop(db, profRef) else IO.unit
   125     } yield ()
   126 
