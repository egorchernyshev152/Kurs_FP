...
### scala.reflect.internal.FatalError: 
  ...
  last tree to typer: Ident(Conference)
       tree position: line 82 of file:///C:/Users/toorr/v1/src/main/scala/MainApp.scala
              symbol: <none>
...
== Source file context for tree position ==
    ...
    79       parse = { case List(n,a,h,av) => for { hi <- h.toIntOption; avl <- av.toLowerCase match { case "y" => Some(true); case "n" => Some(false); case _ => None } } yield Professor(n, a.split(";").toList, hi, avl); case _ => None },
    80       format= p => List(p.name, p.areas.mkString(";"), p.hIndex.toString, if(p.available) "y" else "n")
    81     )
    82     val confRepo: Repository[Conference] = new CsvRepository[Conference](
    83       _CURSOR_parse = { case List(n,r,t) => r.toDoubleOption.map(rd => Conference(n, rd, t.split(";").toList)); case _ => None },
    84       format= c => List(c.name, c.rating.toString, c.topics.mkString(";"))
    85     )
