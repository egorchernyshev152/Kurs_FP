...
### scala.reflect.internal.FatalError: 
  ...
  last tree to typer: Ident(_CURSOR_yield)
   ...
   symbol definition: val <error: <none>>: <error> (a TermSymbol)
      symbol package: <empty>
       symbol owners: value <error: <none>> -> value $anonfun -> value $anonfun -> value $anonfun -> value $anonfun -> method run -> object MainApp
           call site: <none> in <none>

== Source file context for tree position ==

   129       db      <- loader.loadAll()
   130       profRef <- Ref.of[IO, List[Professor]](db.professors)
   131       _       <- appLoop(db, profRef)
   132     } _CURSOR_yield ExitCode.Success
   133   }
   134 }
   135 
