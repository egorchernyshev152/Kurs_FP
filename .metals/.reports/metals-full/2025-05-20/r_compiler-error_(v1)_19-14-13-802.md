...
== Source file context for tree position ==
   ...
   105     )
   106     val artRepo = new CsvRepository[Article](
   107       _CURSOR_parse = { case List(tl, top, doi, y) => y.toIntOption.map(yi => Article(tl, top, if (doi.nonEmpty) Some(doi) else None, yi)); case _ => None },
   108       format = a => List(a.title, a.topic, a.doi.getOrElse(""), a.year.toString)
   109     )
