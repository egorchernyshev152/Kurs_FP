...
### scala.reflect.internal.FatalError: 
  ...
  last tree to typer: Ident(_CURSOR_parse)
       tree position: line 97 of file:///C:/Users/toorr/v1/src/main/scala/MainApp.scala
            tree tpe: <error>
              symbol: value <error: <none>>
   symbol definition: val <error: <none>>: <error> (a TermSymbol)
      symbol package: <empty>
       symbol owners: value <error: <none>> -> value profRepo -> method run -> object MainApp
           call site: <none> in <none>

== Source file context for tree position ==

    94   def run(args: List[String]): IO[ExitCode] = {
    95     // 1) Создаём CSV-репозитории для всех сущностей
    96     val profRepo: Repository[IO, Professor] = new CsvRepository[IO, Professor](
    97       _CURSOR_parse = {
    98         case List(n, areas, h, av) =>
    99           for {
   100             hi  <- h.toIntOption
...
object AdminMenu {
  ...
  def run(profRef: Ref[IO, List[Professor]], profRepo: Repository[IO, Professor]): IO[Unit] = {
    def menu: IO[Unit] = for {
      ...
      _ <- cmd match {
        case "1" => for {
          ...
          areasS <- readUntilValid("Области (через ';', например ml;computer vision):") { s =>
                       Either.cond(s.nonEmpty, s, "Список не пустой")
                     }
          hIndex <- readUntilValid("hIndex (целое число):") { s =>
                       s.toIntOption.toRight("Неверный число")
                     }
...
object MainApp extends IOApp {
  ...
  def run(args: List[String]): IO[ExitCode] = {
    // 1) Создаём CSV-репозитории для всех сущностей
    val profRepo: Repository[IO, Professor] = new CsvRepository[IO, Professor](
      @@parse = {
    ...
    val confRepo: Repository[IO, Conference] = new CsvRepository[IO, Conference](
      parse = {
    ...
    val topRepo: Repository[IO, Topic] = new CsvRepository[IO, Topic](
      parse = { case List(n, tags) => Some(Topic(n, tags.split(";").toList)); case _ => None },
      format = t => List(t.name, t.tags.mkString(";"))
    )

    val artRepo: Repository[IO, Article] = new CsvRepository[IO, Article](
      parse = {
...
scala.reflect.internal.Reporting.abort(Reporting.scala:69)
	...
	scala.collection.immutable.List.collect(List.scala:275)
